...
var ObjectID = Npm.require('mongodb').ObjectID;                                                                        // 3
                                                                                                                       //
var Grid = Npm.require('gridfs-stream'); //var Grid = Npm.require('gridfs-locking-stream');                            // 4
                                                                                                                       //
                                                                                                                       //
...
FS.Store.GridFS = function (name, options) {                                                                           // 22
  ...
  if (!(self instanceof FS.Store.GridFS)) throw new Error('FS.Store.GridFS missing keyword "new"');                    // 29
                                                                                                                       //
  ...
  if (!options.mongoOptions) {                                                                                         // 38
    options.mongoOptions = {                                                                                           // 39
      db: {                                                                                                            // 39
        native_parser: true                                                                                            // 39
      },                                                                                                               // 39
      server: {                                                                                                        // 39
        auto_reconnect: true                                                                                           // 39
      }                                                                                                                // 39
    };                                                                                                                 // 39
  }                                                                                                                    // 40
                                                                                                                       //
  ...
  return new FS.StorageAdapter(name, options, {                                                                        // 46
    ...
    fileKey: function (fileObj) {                                                                                      // 49
      ...
      var key = {                                                                                                      // 52
        _id: null,                                                                                                     // 53
        filename: null                                                                                                 // 54
      }; // If we're passed a fileObj, we retrieve the _id and filename from it.                                       // 52
                                                                                                                       //
      if (fileObj) {                                                                                                   // 58
        var info = fileObj._getInfo(name, {                                                                            // 59
          updateFileRecordFirst: false                                                                                 // 59
        });                                                                                                            // 59
                                                                                                                       //
        key._id = info.key || null;                                                                                    // 60
        key.filename = info.name || fileObj.name({                                                                     // 61
          updateFileRecordFirst: false                                                                                 // 61
        }) || fileObj.collectionName + '-' + fileObj._id;                                                              // 61
      } // If key._id is null at this point, createWriteStream will let GridFS generate a new ID                       // 62
                                                                                                                       //
                                                                                                                       //
      return key;                                                                                                      // 65
    },                                                                                                                 // 66
    createReadStream: function (fileKey, options) {                                                                    // 67
      options = options || {}; // Init GridFS                                                                          // 68
                                                                                                                       //
      var gfs = new Grid(self.db, mongodb); // Set the default streamning settings                                     // 71
                                                                                                                       //
      var settings = {                                                                                                 // 74
        _id: new ObjectID(fileKey._id),                                                                                // 75
        root: gridfsName                                                                                               // 76
      }; // Check if this should be a partial read                                                                     // 74
                                                                                                                       //
    ...
    createWriteStream: function (fileKey, options) {                                                                   // 93
      options = options || {}; // Init GridFS                                                                          // 94
                                                                                                                       //
      ...
      writeStream.on('close', function (file) {                                                                        // 117
        if (!file) {                                                                                                   // 118
          ...
          return;                                                                                                      // 122
                                                                                                                       ...
                                                                                                                       //
        if (FS.debug) console.log('SA GridFS - DONE!'); // Emit end and return the fileKey, size, and updated date     // 125
                                                                                                                       //
