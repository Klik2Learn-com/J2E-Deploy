Template.Yeti_d1_e2.helpers({
	
	activeSection: function(){
		var activeSection = Session.get("activeSection");
		return (activeSection == "#Yeti_d1_e2");
	}
});


Template.Yeti_d1_e2.events({
	"dragstart .dragitem": function(evt) {
		...
		$.k2l.yeti_d1e2.draggedElement = evt.currentTarget;
	},
	...
	"drop .answerSlot": function(evt) {
		...
		if ($($.k2l.yeti_d1e2.draggedElement).parent().hasClass('Yetibox')) {
			$($.k2l.yeti_d1e2.draggedElement).addClass('answerBank');
			$($.k2l.yeti_d1e2.draggedElement).addClass('ddtarget');
			$($.k2l.yeti_d1e2.draggedElement).addClass('ddwidth9');
			$($.k2l.yeti_d1e2.draggedElement).removeClass('ddsourceyeti');
			$($.k2l.yeti_d1e2.draggedElement).html('&nbsp;');
			$($.k2l.yeti_d1e2.draggedElement).attr('draggable', 'False');
		} else {
			// Otherwise it must be dragged from an answerSlot
			$($.k2l.yeti_d1e2.draggedElement).parent().removeClass('yetidd');
			$($.k2l.yeti_d1e2.draggedElement).parent().addClass('ddtarget');
			$($.k2l.yeti_d1e2.draggedElement).parent().removeClass('ddwidth6'); // swap ddwidth6 for 7
			$($.k2l.yeti_d1e2.draggedElement).parent().addClass('ddwidth7');
			$($.k2l.yeti_d1e2.draggedElement).parent().html('&nbsp;');
		}
	...
	"drop .answerBank": function(evt) {
		...
		if ($($.k2l.yeti_d1e2.draggedElement).parent().hasClass('answerSlot')) {
			$($.k2l.yeti_d1e2.draggedElement).parent().removeClass('yetidd');
			$($.k2l.yeti_d1e2.draggedElement).parent().addClass('ddtarget');
			$($.k2l.yeti_d1e2.draggedElement).parent().removeClass('ddwidth6'); // swap ddwidth6 for 7
			$($.k2l.yeti_d1e2.draggedElement).parent().addClass('ddwidth7');
	...
	"click .pagination": function(evt){
		$.k2l.yeti_d1e2.draggedElement = {};
	},
	
	"click #submitButton": function(evt){
		var answer = true;
		if ($('#target1').children().html() != 'In'){
			answer = false;
		}
		if ($('#target2').children().html() != 'like') {
			answer = false;
		}
		if ($('#target3').children().html() != 'up'){
			answer = false;
		}
		if ($('#target4').children().html() != 'near'){
			answer = false;
		}
		...
		if (answer == true) {
			...
			Session.set('d1_e2_result', 'correct'); // Set the result as correct
			$('.correctbig').removeClass('hidden');
			$.k2l.yeti_d1e2.draggedElement = {};
		} else {
			Session.set('d1_e2_result', 'incorrect'); // Set the result as incorrect
...
Template.Yeti_d1_e2.rendered = function() {
	
	if(typeof $.k2l == 'undefined'){
		$.k2l = {};
	};
	
	if (typeof $.k2l.yeti_d1e2 == 'undefined') {
		$.k2l.yeti_d1e2 = {};
	};
